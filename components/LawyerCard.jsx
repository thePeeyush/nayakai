...
const LawyerCard = ({ information }) => {
  ...
  return (
    <div onClick={() => setOpen(!open)} className={`flex flex-col justify-between  rounded-md ${open ? 'shadow-md' : 'shadow-none'} md:max-w-[400px] lg:max-w-[450px] w-full overflow-hidden hover:outline outline-1 outline-gray-200 hover:cursor-pointer hover:shadow-sm`}>
      ...
      <div className={`flex flex-row bg-yellow-600 bg-opacity-30 text-2xl p-3 justify-between ${open ? 'flex' : 'hidden'}`}>
